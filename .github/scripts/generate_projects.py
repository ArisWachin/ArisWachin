...
Theme: matches the profile banner (navy #0A101F, blue #6495ED, cyan #22D3EE,
...
THEMES = {
    "dark": {
        ...
        "CYAN": "#22D3EE", "BLUE": "#6495ED", "BLUE2": "#3B5BDB",
        "EMERALD": "#10B981", "TEXT": "#F8FAFC", "MUTED": "#94A3B8",
        "DIM": "#475569",
        "STROKE": "rgba(100,149,237,0.28)", "STROKE_HI": "rgba(34,211,238,0.5)",
        "STROKE_LO": "rgba(100,149,237,0.22)", "BARLINE": "rgba(255,255,255,0.08)",
        "RING_BG": "rgba(148,163,184,0.15)", "PILL_BG": "rgba(100,149,237,0.22)",
        "PILL_STROKE": "rgba(100,149,237,0.5)", "MONO_TX": "#EAF2FF",
    },
    "light": {
        "BG": "#F8FAFC", "PANEL": "#FFFFFF", "PANEL_BAR": "#F1F5F9",
        "CYAN": "#0891B2", "BLUE": "#2563EB", "BLUE2": "#2563EB",
        "EMERALD": "#059669", "TEXT": "#0F172A", "MUTED": "#475569",
        "DIM": "#94A3B8",
        "STROKE": "rgba(37,99,235,0.30)", "STROKE_HI": "rgba(8,145,178,0.55)",
        "STROKE_LO": "rgba(37,99,235,0.20)", "BARLINE": "rgba(0,0,0,0.08)",
        "RING_BG": "rgba(100,116,139,0.20)", "PILL_BG": "rgba(37,99,235,0.12)",
        "PILL_STROKE": "rgba(37,99,235,0.4)", "MONO_TX": "#FFFFFF",
    },
}

# active palette — set by set_theme(); defaults to dark
BG = PANEL = PANEL_BAR = CYAN = BLUE = BLUE2 = EMERALD = TEXT = MUTED = DIM = None
...
def set_theme(name):
    ...
    for k, v in t.items():
        g[k] = v
    g["DONUT_COLORS"] = [t["BLUE"], t["CYAN"], t["EMERALD"], "#6366F1", "#64748B", "#94A3B8"]
...
def card(p, x, y, idx):
    ...
    if logo:
        ...
        a(f'<g>{float_anim}<rect x="16" y="44" width="40" height="40" rx="9" fill="{BLUE2}" opacity="0.9"/>'
    ...
    for tag in (p.get("tags") or [])[:3]:
        ...
        a(f'<text x="{tx + tw/2:.0f}" y="130" text-anchor="middle" font-size="9.5" fill="{BLUE}">{esc(tag)}</text>')
        tx += tw + 7
...
def build(projects, theme="dark"):
    ...
    a(f'<defs><linearGradient id="{gid}" x1="0" y1="0" x2="1" y2="0">'
      f'<stop offset="0" stop-color="{BLUE2}"><animate attributeName="stop-color" values="{BLUE2};{CYAN};{EMERALD};{BLUE2}" dur="10s" repeatCount="indefinite"/></stop>'
      f'<stop offset="1" stop-color="{EMERALD}"><animate attributeName="stop-color" values="{EMERALD};{BLUE2};{CYAN};{EMERALD}" dur="10s" repeatCount="indefinite"/></stop>'
      '</linearGradient></defs>')
